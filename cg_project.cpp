...
float farBuildingsX = 0.0f;
float nearBuildingsX = 0.0f;
...
struct Building {
    ...
    float height;
    Color color;
    std::vector<bool> windows; // Track which windows are lit (static)
};

std::vector<Building> farBuildings;
std::vector<Building> nearBuildings;

void initializeBuildingWindows(Building& building) {
    int windowRows = building.height / 40;
    int windowCols = building.width / 30;
    building.windows.clear();

    for (int i = 0; i < windowRows * windowCols; i++) {
        // Set windows to be lit based on a pattern (not random)
        building.windows.push_back((i % 4) != 0); // 3 out of 4 windows are lit
    }
}

void initializeBuildings() {
    // Create distant buildings (smaller, less detailed)
    for (int i = 0; i < 10; i++) {
        Building b;
        b.x = i * 250 + (rand() % 100);
        b.width = 80 + rand() % 120;
        b.height = 100 + rand() % 150;
        b.color = { 0.3f + (rand() % 70) / 100.0f, 0.3f + (rand() % 70) / 100.0f, 0.3f + (rand() % 70) / 100.0f };
        initializeBuildingWindows(b);
        farBuildings.push_back(b);
    }

    // Create near buildings (larger, more detailed)
    for (int i = 0; i < 8; i++) {
        Building b;
        b.x = i * 300 + (rand() % 150);
        b.width = 120 + rand() % 180;
        b.height = 150 + rand() % 200;
        b.color = { 0.4f + (rand() % 60) / 100.0f, 0.4f + (rand() % 60) / 100.0f, 0.4f + (rand() % 60) / 100.0f };
        initializeBuildingWindows(b);
        nearBuildings.push_back(b);
    }
}
...
void drawFarBuildings() {
    for (auto& building : farBuildings) {
        float xPos = building.x + farBuildingsX;

        // Wrap around when buildings go off screen
        if (xPos < -building.width) {
            xPos = 800 + (rand() % 200);
            building.x = xPos - farBuildingsX;
            initializeBuildingWindows(building);
        }

        // Main building
        glColor3f(building.color.r, building.color.g, building.color.b);
        glBegin(GL_QUADS);
        glVertex2f(xPos, groundY);
        glVertex2f(xPos + building.width, groundY);
        glVertex2f(xPos + building.width, groundY + building.height);
        glVertex2f(xPos, groundY + building.height);
        glEnd();

        // Windows (static pattern)
        ...
        for (int row = 0; row < windowRows; row++) {
            for (int col = 0; col < windowCols; col++) {
                int index = row * windowCols + col;
                if (index < building.windows.size() && building.windows[index]) {
                    glBegin(GL_QUADS);
                    glVertex2f(xPos + 10 + col * 30, groundY + 10 + row * 40);
                    glVertex2f(xPos + 20 + col * 30, groundY + 10 + row * 40);
                    glVertex2f(xPos + 20 + col * 30, groundY + 25 + row * 40);
                    glVertex2f(xPos + 10 + col * 30, groundY + 25 + row * 40);
                    glEnd();
                }
            }
        }
...
void drawNearBuildings() {
    for (auto& building : nearBuildings) {
        float xPos = building.x + nearBuildingsX;

        // Wrap around when buildings go off screen
        if (xPos < -building.width) {
            xPos = 800 + (rand() % 300);
            building.x = xPos - nearBuildingsX;
            initializeBuildingWindows(building);
        }
        ...
        int windowRows = building.height / 30;
        int windowCols = building.width / 25;
        ...
        // Building details
        glColor3f(building.color.r * 0.8f, building.color.g * 0.8f, building.color.b * 0.8f);
        glBegin(GL_QUADS);
        glVertex2f(xPos, groundY + building.height);
        glVertex2f(xPos + building.width, groundY + building.height);
        glVertex2f(xPos + building.width, groundY + building.height + 5);
        glVertex2f(xPos, groundY + building.height + 5);
        glEnd();
    }
}
...
void display() {
    ...
    drawSun();
    drawFarBuildings();  // Distant buildings (move slower)
    drawMovingClouds();
    drawNearBuildings(); // Closer buildings (move faster)
    drawDetailedGround();
    drawAnimatedDino();
...
void update(int value) {
    if (!isGameOver) {
        ...
        // Parallax building movement
        farBuildingsX -= 0.5f;  // Distant buildings move slower
        nearBuildingsX -= 1.2f;  // Near buildings move faster
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("T-Rex Runner with Parallax Buildings");
